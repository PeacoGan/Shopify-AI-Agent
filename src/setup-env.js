...
function renderEnv(env) {
  const lines = [
    ...
    `FEISHU_FIELD_SHOPIFY_PRODUCT_ID=${
      env.FEISHU_FIELD_SHOPIFY_PRODUCT_ID || 'Shopify 商品ID'
    }`,
    `FEISHU_FIELD_SHOPIFY_HANDLE=${env.FEISHU_FIELD_SHOPIFY_HANDLE || 'Shopify Handle'}`,
    `FEISHU_FIELD_SOURCE_DOC_URLS=${env.FEISHU_FIELD_SOURCE_DOC_URLS || '资料文档链接'}`,
    `FEISHU_FIELD_SOURCE_NOTES=${env.FEISHU_FIELD_SOURCE_NOTES || '资料备注'}`,
    `FEISHU_FIELD_TARGET_MARKET=${env.FEISHU_FIELD_TARGET_MARKET || '目标市场'}`,
    `FEISHU_FIELD_BRAND_VOICE=${env.FEISHU_FIELD_BRAND_VOICE || '品牌语气'}`,
    `FEISHU_FIELD_APPROVAL_STATUS=${env.FEISHU_FIELD_APPROVAL_STATUS || '审批状态'}`,
    `FEISHU_FIELD_DRAFT_JSON=${env.FEISHU_FIELD_DRAFT_JSON || 'AI草稿JSON'}`,
    `FEISHU_FIELD_DIFF_JSON=${env.FEISHU_FIELD_DIFF_JSON || 'Shopify差异JSON'}`,
    `FEISHU_FIELD_LAST_ERROR=${env.FEISHU_FIELD_LAST_ERROR || '最近错误'}`,
    `FEISHU_FIELD_LAST_SYNCED_AT=${env.FEISHU_FIELD_LAST_SYNCED_AT || '最近同步时间'}`,
    '',
    `FEISHU_STATUS_READY=${env.FEISHU_STATUS_READY || '待生成'}`,
    `FEISHU_STATUS_PENDING_APPROVAL=${
      env.FEISHU_STATUS_PENDING_APPROVAL || '待审批'
    }`,
    `FEISHU_STATUS_APPROVED=${env.FEISHU_STATUS_APPROVED || '已批准'}`,
    `FEISHU_STATUS_APPLIED=${env.FEISHU_STATUS_APPLIED || '已同步'}`,
    `FEISHU_STATUS_ERROR=${env.FEISHU_STATUS_ERROR || '错误'}`,
    '',
